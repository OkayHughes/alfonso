...
function intParams = FeketeCubeRand(n,d, percent)
...
% -------------------------------------------------------------------------
    ...
    w = P'\m;
    ind = abs(w)>0;
    ind_inds = 1:size(ind, 1);
    ind_inds = ind_inds(ind)';
    rind = abs(w) <= 0;
    rind_inds = 1:size(rind, 1);
    rind_inds = rind_inds(rind)';
    amt = floor(size(ind_inds, 1) * percent);
    rand_sample = randsample(size(ind_inds, 1), amt);
    ind(ind_inds(rand_sample)) = 0;
    rand_sample = randsample(size(rind_inds, 1), amt);
    ind(rind_inds(rand_sample)) = 1;
